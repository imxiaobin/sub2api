export default {
    accounts: {
      ...
      duplicate: '复制',
      duplicating: '复制中...',
      duplicateSuccess: '账号已复制为「{name}」',
      duplicateSharedCredentialSuccess: '账号已复制为「{name}」。副本与原账号共用同一份 OAuth 凭据：任一方刷新令牌后若上游轮换了 token，另一方会失效并需要重新授权。',
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。新建与导入的账号默认「仅设备」，可在此改成其他档位或关闭；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
        codexFingerprintConvergence: '实验性指纹收敛（klno）',
        codexFingerprintConvergenceDesc: '让这把 API key 的出站身份在 HTTP / WS 上与真 Codex 客户端形态一致：补齐 session-id / thread-id 头，x-client-request-id 等于 thread-id，去掉 session_id / conversation_id 别名，root_turn_id 等与同类字段同源派生，保持 UUIDv7。关闭时与上游行为完全一致；开启那一刻该账号的会话标识会一次性轮换。新建与导入的账号默认开启。',
